 ...
 */
public class TermsQueryBuilder implements QueryBuilder, SpanQueryBuilder {
  ...
  private class TermsQueryProcessor implements TermBuilder.TermProcessor {
    ...
    private List<SpanTermQuery> listStq = null; // this will be instantiated only if the TermsQuery results in multiple terms
    private SpanTermQuery firstStq = null;//Keeps the first SpanTermQuery for the first Term in the query and if there are more terms found then this will be consumed by above BooleanQuery
    ...
    public void process(Term t){
      ...
      if (listStq == null) {
        listStq = new LinkedList<SpanTermQuery>();
      }
      listStq.add(new SpanTermQuery(t));
    }
    ...
    public SpanQuery getSpanQuery() {
      if (firstStq == null) {
          // TODO Fix this
          return null; // new MatchAllDocsQuery();
      } else if (bq == null) {
          float boost = DOMUtils.getAttribute(xmlQueryElement, "boost", 1.0f);
          if (boost != 1f)
          {
            return new SpanBoostQuery(firstStq, boost);
          }
          return firstStq;
      } else {
          SpanQuery q = new SpanOrQuery(listStq.toArray(new SpanQuery[listStq.size()]));
          float boost = DOMUtils.getAttribute(xmlQueryElement, "boost", 1.0f);
          if (boost != 1f)
          {
